...
### 7. ChannelManager への追加メソッド移動

- [x] `lockDisplayToSchedule()`を移動
- [x] `evaluateScheduleConflict()`を移動
- [x] `applyAssignmentLocally()`を移動
- [x] `computeConflictSignature()`を移動
- [x] `isConflictDialogSnoozed()`を移動
- [x] `scheduleConsensusAdoption()`を移動
- [x] `applyConsensusAdoption()`を移動
- [x] `refreshChannelSubscriptions()`を移動
- [x] `app.js`でメソッドを`ChannelManager`に委譲
- [x] 古い実装を削除
...
- `app.js`: 5,040 行 → **2,463 行**（約 2,577 行削減、51.1%削減）
- 新規作成ファイル:
  - `ui-helpers.js`: 228 行
  - `context-manager.js`: 332 行
  - `auth-manager.js`: 358 行
  - `presence-manager.js`: 753 行
  - `channel-manager.js`: 1,314 行
  - `ui-renderer.js`: 378 行

### 削減効果

- 合計削減: 約 2,577 行（`app.js`から）
- 新規追加: 約 3,363 行（6 つのマネージャーファイル）
- 実質削減: 約 2,577 行（`app.js`の可読性向上）
