...
現在のデータ構造では、ID があるにも関わらず、その ID に紐づく情報（イベント名、スケジュールラベルなど）が複数箇所に重複保存されています。これにより以下の問題が発生しています：
...
1. **フェーズ 1: アプローチ 3（一括同期）を実装**

   - 既存コードへの影響が少ない
   - データ整合性の問題を緩和

2. **フェーズ 2: アプローチ 2（ハイブリッド）に移行**

   - 新規作成時は正規化された構造を使用
   - 既存データは段階的に移行

3. **フェーズ 3: アプローチ 1（完全正規化）を検討**
   - パフォーマンスへの影響を評価
   - 必要に応じて完全正規化を実装
