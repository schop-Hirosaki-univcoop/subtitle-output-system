...
このドキュメントは、`scripts/question-admin/app.js`（8,180 行 →5,752 行）のリファクタリング計画をまとめます。

## 現状分析

### ファイルサイズ

- `scripts/question-admin/app.js`: 8,180 行（開発標準の約 5 倍）→ 5,752 行（リファクタリング進行中）
...
- [x] フェーズ 4: 参加者管理機能の分離 ✅ 完了
- [x] フェーズ 5: 日程管理機能の分離 ✅ 完了
...
- `app.js`: 6,023 行 → 5,752 行（約 271 行削減、最終的な現在の行数）
