...
| `scripts/events/panels/gl-panel.js`                      | 1,815 | ❌ 要改善（基準の約 1.2 倍、リファクタリング進行中、段階 1-2 完了）    |
...
│   ├── gl-panel.js           # GlToolManager（1,815行、リファクタリング進行中、段階1-2完了）⚠️
...
2. **`panels/gl-panel.js` が大きい（1,815 行、リファクタリング進行中）**
...
3. **`scripts/events/panels/gl-panel.js` が 1,815 行（リファクタリング進行中、段階 1-2 完了）**
   - 開発標準の約 2 倍
   - 分割を検討すべき
...
- ⚠️ `scripts/events/panels/gl-panel.js` - 1,815 行、要改善（リファクタリング進行中、段階 1-2 完了、元の 3,249 行から約 1,434 行削減、約 44% 削減）
...
2. **中期的に対応**: `scripts/events/panels/gl-panel.js` と `scripts/gl-form/index.js` のリファクタリング
   - `scripts/events/panels/gl-panel.js`: リファクタリング進行中（段階 1-2 完了、約 941 行削減、約 29% 削減）
     - `gl-panel.js`: 3,249 行 → 1,815 行（約 44% 削減）
...
| GL 管理パネル               | `scripts/events/panels/gl-panel.js` (1,815 行)     | ✅ 対応  | ファイルが大きいが対応している（リファクタリング進行中、段階 1-2 完了） |
...
│   ├── gl-panel.js            # GL管理パネル（1,815行、要改善、リファクタリング進行中、段階1-2完了）
│   ├── gl-utils.js            # ユーティリティ（789行）✅
│   ├── gl-renderer.js         # UI描画（982行、段階2完了）✅
