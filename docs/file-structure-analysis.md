...
| `scripts/events/panels/gl-panel.js`                      | 2,308 | ❌ 要改善（基準の約 1.5 倍、リファクタリング進行中、段階 1-2 完了）    |
| `scripts/events/panels/gl-utils.js`                      | 789   | ✅ 許容範囲（新規作成、フェーズ 2 段階 1 完了）                        |
| `scripts/events/panels/gl-renderer.js`                   | 518   | ✅ 許容範囲（新規作成、フェーズ 2 段階 2 進行中）                      |
...
│   ├── gl-panel.js           # GlToolManager（2,308行、リファクタリング進行中、段階1-2完了）⚠️
│   ├── gl-utils.js           # ユーティリティ（789行）✅
│   ├── gl-renderer.js        # UI描画（518行、段階2進行中）✅
...
2. **`panels/gl-panel.js` が大きい（2,308 行、リファクタリング進行中）**
   - GL ツールの機能が単一ファイルに集約（元の 3,249 行から約 941 行削減、約 29% 削減）
   - ✅ 段階 1: ユーティリティ関数を `gl-utils.js` に移行完了
   - ✅ 段階 2: UI 描画機能を `gl-renderer.js` に移行完了（進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個 + 応募者・スケジュール関連 2 個の基本構造を移行）
   - ⏳ 分割を継続中
...
3. **`scripts/events/panels/gl-panel.js` が 2,308 行（リファクタリング進行中、段階 1-2 完了）**
   - 開発標準の約 2 倍
   - 分割を検討すべき
...
├── gl-application-manager.js # 応募管理（800行程度予定）
├── gl-assignment-manager.js  # 割り当て管理（600行程度予定）
├── gl-renderer.js            # UI 描画（700行程度予定、518行、段階2進行中）✅ 進行中
...
3. **`scripts/events/panels/gl-panel.js` のリファクタリング**（進行中）
   ...
   - **進捗状況**:
     ...
     - ✅ 段階 2: UI 描画機能を `gl-renderer.js` に移行（進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個 + 応募者・スケジュール関連 2 個の基本構造を移行）
       - `gl-panel.js`: 2,526 行 → 2,368 行（約 158 行削減、約 6.7% 削減）
       - `gl-panel.js`: 2,368 行 → 2,302 行（約 66 行削減、約 2.8% 削減）
       - `gl-panel.js`: 2,302 行 → 2,308 行（約 6 行追加、コメント追加）
       - `gl-renderer.js`: 265 行 → 385 行（約 120 行追加、スケジュール班関連の描画メソッド 2 個を実装）
       - `gl-renderer.js`: 385 行 → 518 行（約 133 行追加、応募者・スケジュール関連の描画メソッド 2 個の基本構造を実装）
       - 実績: 約 941 行の削減（`gl-panel.js` は 2,308 行、`gl-renderer.js` は 518 行、段階 2 進行中）
       - 移行完了: 内部スタッフ関連 5 個、スケジュール班関連 2 個（計 7 個）
       - 進行中: 応募者・スケジュール関連 2 個の基本構造（`renderScheduleBoard`、`renderApplicantList`）
       - 残りの描画メソッド（依存メソッド含む約 7 個）を移行予定
     - ⏳ 段階 3: 応募管理機能を `gl-application-manager.js` に移行（予定）
     - ⏳ 段階 4: 割り当て管理機能を `gl-assignment-manager.js` に移行（予定）
     - ⏳ 段階 5: GlToolManager を整理し、メインファイルを縮小（予定）
...
- ⚠️ `scripts/events/panels/gl-panel.js` - 2,308 行、要改善（リファクタリング進行中、段階 1-2 完了、元の 3,249 行から約 941 行削減、約 29% 削減）
- ✅ `scripts/events/panels/gl-utils.js` - 789 行（新規作成、フェーズ 2 段階 1 完了）
- ✅ `scripts/events/panels/gl-renderer.js` - 518 行（新規作成、フェーズ 2 段階 2 進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個 + 応募者・スケジュール関連 2 個の基本構造を実装）
...
2. **中期的に対応**: `scripts/events/panels/gl-panel.js` と `scripts/gl-form/index.js` のリファクタリング
   - `scripts/events/panels/gl-panel.js`: リファクタリング進行中（段階 1-2 完了、約 941 行削減、約 29% 削減）
     - `gl-panel.js`: 3,249 行 → 2,308 行（約 29% 削減）
     - `gl-utils.js`: 789 行（新規作成、段階 1 完了）
     - `gl-renderer.js`: 518 行（新規作成、段階 2 進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個 + 応募者・スケジュール関連 2 個の基本構造を実装）
...
| GL 管理パネル               | `scripts/events/panels/gl-panel.js` (2,308 行)     | ✅ 対応  | ファイルが大きいが対応している（リファクタリング進行中、段階 1-2 完了） |
...
│   ├── gl-panel.js            # GL管理パネル（2,308行、要改善、リファクタリング進行中、段階1-2完了）
│   ├── gl-utils.js            # ユーティリティ（789行）✅
│   ├── gl-renderer.js         # UI描画（518行、段階2進行中）✅
