...
| `scripts/events/panels/gl-panel.js`                      | 2,302 | ❌ 要改善（基準の約 1.5 倍、リファクタリング進行中、段階 1-2 完了）    |
| `scripts/events/panels/gl-utils.js`                      | 789   | ✅ 許容範囲（新規作成、フェーズ 2 段階 1 完了）                        |
| `scripts/events/panels/gl-renderer.js`                   | 385   | ✅ 許容範囲（新規作成、フェーズ 2 段階 2 進行中）                      |
...
│   ├── gl-panel.js           # GlToolManager（2,302行、リファクタリング進行中、段階1-2完了）⚠️
│   ├── gl-utils.js           # ユーティリティ（789行）✅
│   ├── gl-renderer.js        # UI描画（385行、段階2進行中）✅
...
2. **`panels/gl-panel.js` が大きい（2,302 行、リファクタリング進行中）**
   - GL ツールの機能が単一ファイルに集約（元の 3,249 行から約 947 行削減、約 29% 削減）
   - ✅ 段階 1: ユーティリティ関数を `gl-utils.js` に移行完了
   - ✅ 段階 2: UI 描画機能を `gl-renderer.js` に移行完了（進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個を移行）
   - ⏳ 分割を継続中
...
3. **`scripts/events/panels/gl-panel.js` が 2,302 行（リファクタリング進行中、段階 1-2 完了）**
   - 開発標準の約 2 倍
   - 分割を検討すべき
...
├── gl-renderer.js            # UI 描画（700行程度予定、385行、段階2進行中）✅ 進行中
...
3. **`scripts/events/panels/gl-panel.js` のリファクタリング**（進行中）
   ...
   - **進捗状況**:
     ...
     - ✅ 段階 2: UI 描画機能を `gl-renderer.js` に移行（進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個を移行完了）
       - `gl-panel.js`: 2,526 行 → 2,368 行（約 158 行削減、約 6.7% 削減）
       - `gl-panel.js`: 2,368 行 → 2,302 行（約 66 行削減、約 2.8% 削減）
       - `gl-renderer.js`: 265 行 → 385 行（約 120 行追加、スケジュール班関連の描画メソッド 2 個を実装）
       - 実績: 約 224 行の削減（`gl-panel.js` は 2,302 行、`gl-renderer.js` は 385 行、段階 2 進行中）
       - 移行完了: 内部スタッフ関連 5 個、スケジュール班関連 2 個（計 7 個）
       - 残りの描画メソッド（10 個）を移行予定
...
- ⚠️ `scripts/events/panels/gl-panel.js` - 2,302 行、要改善（リファクタリング進行中、段階 1-2 完了、元の 3,249 行から約 947 行削減、約 29% 削減）
- ✅ `scripts/events/panels/gl-utils.js` - 789 行（新規作成、フェーズ 2 段階 1 完了）
- ✅ `scripts/events/panels/gl-renderer.js` - 385 行（新規作成、フェーズ 2 段階 2 進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個を実装）
...
2. **中期的に対応**: `scripts/events/panels/gl-panel.js` と `scripts/gl-form/index.js` のリファクタリング
   - `scripts/events/panels/gl-panel.js`: リファクタリング進行中（段階 1-2 完了、約 947 行削減、約 29% 削減）
     - `gl-panel.js`: 3,249 行 → 2,302 行（約 29% 削減）
     - `gl-utils.js`: 789 行（新規作成、段階 1 完了）
     - `gl-renderer.js`: 385 行（新規作成、段階 2 進行中、内部スタッフ関連 5 個 + スケジュール班関連 2 個を実装）
...
| GL 管理パネル               | `scripts/events/panels/gl-panel.js` (2,302 行)     | ✅ 対応  | ファイルが大きいが対応している（リファクタリング進行中、段階 1-2 完了） |
...
│   ├── gl-panel.js            # GL管理パネル（2,302行、要改善、リファクタリング進行中、段階1-2完了）
│   ├── gl-utils.js            # ユーティリティ（789行）✅
│   ├── gl-renderer.js         # UI描画（385行、段階2進行中）✅
