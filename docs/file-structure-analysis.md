...
| `scripts/events/panels/gl-panel.js`                      | 1,768 | ❌ 要改善（基準の約 1.2 倍、リファクタリング進行中、段階 1-3 完了）    |
...
| `scripts/events/panels/gl-application-manager.js`        | 157   | ✅ 許容範囲（新規作成、フェーズ 2 段階 3 進行中）                      |
...
│   ├── gl-panel.js           # GlToolManager（1,768行、リファクタリング進行中、段階1-3完了）⚠️
│   ├── gl-utils.js           # ユーティリティ（789行）✅
│   ├── gl-renderer.js        # UI描画（982行、段階2完了）✅
│   ├── gl-application-manager.js # 応募管理（157行、段階3進行中）✅
...
2. **`panels/gl-panel.js` が大きい（1,768 行、リファクタリング進行中）**
...
3. **`scripts/events/panels/gl-panel.js` が 1,768 行（リファクタリング進行中、段階 1-3 完了）**
   - 開発標準の約 2 倍
   - 分割を検討すべき
...
- ⚠️ `scripts/events/panels/gl-panel.js` - 1,768 行、要改善（リファクタリング進行中、段階 1-3 完了、元の 3,249 行から約 1,481 行削減、約 46% 削減）
...
2. **中期的に対応**: `scripts/events/panels/gl-panel.js` と `scripts/gl-form/index.js` のリファクタリング
   - `scripts/events/panels/gl-panel.js`: リファクタリング進行中（段階 1-3 完了、約 1,481 行削減、約 46% 削減）
     - `gl-panel.js`: 3,249 行 → 1,768 行（約 46% 削減）
     ...
     - `gl-application-manager.js`: 157 行（新規作成、段階 3 進行中、応募データの読み込み・保存・削除機能を実装）
...
| GL 管理パネル               | `scripts/events/panels/gl-panel.js` (1,768 行)     | ✅ 対応  | ファイルが大きいが対応している（リファクタリング進行中、段階 1-3 完了） |
...
│   ├── gl-panel.js            # GL管理パネル（1,768行、要改善、リファクタリング進行中、段階1-3完了）
│   ├── gl-utils.js            # ユーティリティ（789行）✅
│   ├── gl-renderer.js         # UI描画（982行、段階2完了）✅
