...
| ファイル                                      | 行数  | 評価                                                 |
| --------------------------------------------- | ----- | ---------------------------------------------------- |
| `scripts/events/app.js`                       | 7,716 | ❌ 要改善（基準の約 5.1 倍、リファクタリング進行中） |
...
2. **巨大な単一ファイル**
   ...
   - `scripts/events/app.js` - 7,716 行（リファクタリング進行中、元の 10,180 行から約 2,464 行削減）
...
- `app.js` を `scripts/operator/` と同様に Manager パターンで分割（進行中）
  ...
  - ✅ `EventFirebaseManager` - Firebase 操作（774 行、基本実装完了、確認中）
...
1. **`scripts/events/app.js` が 7,716 行（リファクタリング進行中）**

   - 開発標準の約 5.1 倍（元の 10,180 行から約 2,464 行削減）
   - イベント管理パネルと日程管理パネルを分離済み（`event-panel.js`, `schedule-panel.js`）
   - 単一責任の原則に違反（改善中）
   - テストが困難（改善中）
   - 保守性が低い（改善中）
...
- ⏳ フェーズ 1.6: 重複メソッドの削除とクリーンアップ（進行中）
  ...
  - ⏳ フェーズ 1.6.5: 最終確認とドキュメント更新（未着手）
    ...
    - ドキュメントを更新
    - `app.js` の行数を確認（現在: 7,716 行、フェーズ 1.5.1 完了により約 46 行削減、フェーズ 1.5.2 完了により約 300 行削減、未使用定数・インポート削除により約 2 行削減）
...
1. **`scripts/events/app.js` のリファクタリング**（進行中）
   ...
   - **残りのフェーズ**:
     - ⏳ フェーズ 1.7: スケジュールコンフリクト管理機能の整理（未着手）
       ...
       - 見積もり: 約 200-300 行の削減
     - ⏳ フェーズ 1.8: スケジュールコンフリクトダイアログ機能の整理（未着手）
       - `openScheduleConflictDialog`, `handleScheduleConflictSubmit`, `confirmScheduleConsensus`, `requestScheduleConflictPrompt` などの整理
       - 見積もり: 約 150-200 行の削減
...
- ❌ `scripts/events/app.js` - 7,716 行、要改善（リファクタリング進行中、元の 10,180 行から約 2,464 行削減）
...
- `scripts/events/app.js` (7,716 行) から両方のパネルの実装を分離済み
